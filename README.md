...
- `false` (`'N'`) — после создания смарт-процесса **всем ролям (включая администраторские) выставляются нулевые права на его воронки**: все записи прав удаляются, доступа нет ни у кого. Это перекрывает дефолтное поведение ядра Bitrix, которое при создании воронки автоматически выдаёт права ролям по их пресетам (MANAGER → «свои», DEPUTY → «отдел», HEAD → «все», ADMIN → «все»).
- `true` (`'Y'`) — поведение ядра не меняется: права выдаются как обычно.

Закрытие выполняется **фоновой задачей** (поставленной в очередь ПОСЛЕ задачи ядра, которая выдаёт права), чтобы гарантированно перекрыть выдачу прав ядром. Используется штатный API ядра `CCrmRole::EraseEntityPermissons()` для каждой воронки — удаление всех записей прав означает нулевой доступ у всех ролей. Определение админ-ролей не выполняется: при `N` права закрываются у всех без исключения.